...
export class BulkUploadComponent implements OnInit {
  ...
  cancel() {
    if (this.role === 'Vendor') {
      this.router.navigateByUrl('/vendor-products-list').then(() => {
        window.location.reload();
      });
    } else if (this.role === 'SuperAdmin') {
      this.router
        .navigate(['/super-vendor-list/super-vendor-detail/product-list'], {
          queryParams: {
            id: this.vendorId,
            id2: this.shopId,
          },
  ...
  getExcelSteetByMainOrSubOrSubSubCategory() {
    ...
    this.content
      .getExcelByMainOrSubOrSubSubCategory(payload)
      .subscribe((response) => {
        if (response.messages) {
          this.downloadExcel = response.data.excelLink;
        } else {
  ...
  fetchAllData(): void {
    if (this.subSubCategoryId == this.form.value.subsubProductCategoryId) {
      ...
      const getExcelSteetByMainOrSubOrSubSubCategory$: Observable<any> =
        this.content.getExcelByMainOrSubOrSubSubCategory(payload3); // Replace with your GET function 3
      ...
      const getExcelSteetByMainOrSubCategory$: Observable<any> =
        this.content.getExcelByMainOrSubCategory(payload2); // Replace with your GET function 2
      ...
      const getExcelSteetByMainCategory$: Observable<any> =
        this.content.getExcelByMainCategory(payload1); // Replace with your GET function 1
